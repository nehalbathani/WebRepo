	...
	for(var jqlen = 0, uniqueStatLen = 0, uniqueSourceLen = 0, uniqueAcLen = 0, 
		...
		if (response[jqlen].status != "" && !isUnique(response[jqlen].status, allStatus)) {
    		statusHtml += "<div><input type='checkbox' name='" + 
    		              response[jqlen].status + 
    		              "' value='" + response[jqlen].status + 
    		              "' class='status'>&nbsp;" + 
    		              response[jqlen].status + "</input></div>";
...
//to add new status after resend/discard or refresh
function includeNewStatus(jqGridData) {
	var statusHtml = "";
	//TODO make the variable names appropriate to action
	...
	for(var len = 0; len < dataToRemove.length; len++) {
		...
		$(nm).parent().remove();
		//TODO verify if index in splice is right
		//remove from array
		allStatus.splice(len,1);
	}*/
	
	//add to filter for new data
...
function includeNewSource(jqGridData) {
	...
	for(var len = 0, statLen = allSources.length; len < jqGridData.length; len++) {
		...
		if (source != "" && !isUnique(source, allSources)) {
			html += "<div><input type='checkbox' name='" + 
						source + "' value='" + source + 
                          "' class='source'>&nbsp;" + 
                          source + "</input></div>";
...
function includeNewTradeDate(jqGridData) {
	...
	//TODO-Caliry how it works without attaching the select event
