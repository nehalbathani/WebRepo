...
function regionsOptions() {
  let options = `<option value="">All Regions</option>`;
  Object.keys(REGIONS).map((e) => {
    options += `<option value="${e}">${REGIONS[e]}</option>`;
  });
  $("select#selectedRegion").html(options);
  console.log($("select#selectedRegion"));
}

$(document).ready(function () {
  regionsOptions(); // Append Options to Regions
  ...
  $("input.form-check-input").click(function (e) {
    if ($("input.form-check-input:checked").length === 4) {
  ...
  $("input.location-options").click(function (e) {
  ...
  $("#flexCheckAll").click(function () {
    ...
    if (checked) {
      ...
      $("#flexCheckMaui").prop("checked", false);
      $("#flexCheckKauai").prop("checked", false);
      $("#flexCheckOahu").prop("checked", false);
      $("#flexCheckBigIsland").prop("checked", false);
    }
  });
...
function intialMessage() {
  setTimeout(() => {
    addBotMessage(
      "Hi, I'm Iris! Ask me anything about things to do in Hawaii. I'm happiest helping."
    );
  }, 1000);
}
...
function handleTextAreaEvents(e) {
  ...
  13 == e.keyCode &&
    ...
    !1 === e.shiftKey &&
    !1 === e.metaKey &&
    (sendMessage(text.val().trim()), "", text.val(""), e.preventDefault());
  var n = text.val();
  if (38 === e.keyCode && !n) {
...
const url = "https://www.chatbase.co";
...
function sendMessage(inputText) {
  const added = addClientMessage(inputText);
  scrollBottom();
  doAddHistory(inputText, "USER");

  const checkboxInputs = $("input.form-check-input:checked");
  const selectedCheckBoxes = [];
  if (checkboxInputs.length > 0 && checkboxInputs.length < 4) {
  ...
  const prev = [];
  $(".conversation-item.clearfix.not-mine.botText .text-content").map(
    (i, e) => {
      prev.push({
        content: $(e).text(),
        role: "assistant",
  ...
  fetch(url + "/api/v1/chat", {
    ...
    body: JSON.stringify({
      messages: [
        ...prev.slice(prev.length - 3, prev.length),
        { content: `${inputText} ${selectedCheckBoxes.join()}`, role: "user" },
      ],
      chatId: CHAT_ID,
      stream: true,
      model: CHAT_MODEL,
    ...
    .then(async (response) => {
      ...
      while (!done) {
        ...
        message += chunkValue;
        scrollBottom();
        let messageT = recreateMessage(message);
        $("#current-typing #bot-typing-message").html(messageT);

        if (done) {
          doAddHistory(message, "BOT");
          $("#current-typing").removeAttr("id");
          $(".send").prop("disabled", false);
          $(".txt-common").prop("disabled", false);
        }
      }
...
function recreateMessage(message) {
  return (
    message &&
    message.replace(URL_REGAX, function (url) {
...
function addClientMessage(text) {
  var str =
    '<div class="conversation-item clearfix mine clientText" tabindex="-1" aria-label="" role="group">\
			<div class="media">\
...
function addBotMessageHTML() {
  ...
  if ($("#current-typing").length > 0) {
    return;
  }
  var str = `<div class="conversation-item clearfix not-mine botText" id="current-typing" tabindex="-1" aria-label="Bot sent, Text.Absolutely delightful!" role="group">
  <div class="media">
    <div class="media-pic" data-initial="">
      <span class="profile-pic" aria-hidden="true">
        <img draggable="false" alt="Chat bot avatar" src="./assets/img/avatar_head.png">
      </span>
    </div>
    <div class="media-body">
      <header class="messages__media__header">
    ...
    <div class="insight-toggle" title="Message Insight" data-message-uuid="3c819e09-a192-490f-88d6-dbfdd2d45844"></div>
      ...
      <div class="message-wrap">
         <p class="desc text-content" id="bot-typing-message"></p>
         <div class="time-stamp">
            <div class="ack-wrapper">
...
function addBotMessage(text) {
  ...
  let newText =
    ...
    text.replace(urlRegex, function (url) {
      return '<a target="_blank" href="' + url + '">' + url + "</a>"; // return the replaced text
    });

  var str =
    '<div class="conversation-item clearfix not-mine botText" request-message-id="3c819e09-a192-490f-88d6-dbfdd2d45844" tabindex="-1" aria-label="Bot sent, Text.Absolutely delightful!" role="group">\
    <div class="media">\
	    <div class="media-pic" data-initial="">\
        <span class="profile-pic" aria-hidden="true">\
        	<img draggable="false" alt="Chat bot avatar" src="./assets/img/avatar_head.png">\
...
function showBotType(e) {
  ...
  if (!showBotTyping) {
    ...
    var o = document.createElement("div");
    o.innerHTML =
      '<div class="conversation-item not-mine" id="bot-typing">\
    ...
    showBotTyping = true;
    $(r).appendTo($("#messages-list")),
      showBotTyping && clearTimeout(showBotTyping),
      (showBotTyping = setTimeout(function () {
        removeBotTyping();
      }, 500000));
  } else {
    removeBotTyping();
...
function scrollBottom() {
  $("#message-list-region").scrollTop(
    $("#message-list-region")[0].scrollHeight
  );
}
...
function isLastMessageInSequence() {
  ...
  e &&
    e.message &&
    e.message.sequence &&
    e.message.sequence.split("/")[0] != e.message.sequence.split("/")[1] &&
    (t = !0);
  return !t;
}
...
function addMenuList() {
  var menu =
    '<div class="conversation-item clearfix not-mine" id="message-first" request-message-id="fa80d49a-c7cd-40b5-8958-3014580c4305" tabindex="-1" aria-label="Bot sent, Card., Overall, how would you rate your interaction with me today?, actions available. Use tab to navigate through the items., " role="group" >\
                <div class="media">\
									<div class="media-pic" data-initial="">\
										<span class="profile-pic" aria-hidden="true">\
											<img draggable="false" alt="Chat bot avatar" src="./assets/img/avatar_head.png">\
																	...
																	<div class="composer">\
																		<div class="composer__container__preview__container">\
																			...
																			<div class="custom-select">\
																				<input type="text" class="form-control" placeholder="Select an option" id="feedback_type" val="">\
																				<ul>\
																				<li value="0"></li>\
																...
																<div class="composer">\
																	<div class="composer__container__preview__container">\
																		...
																		<div class="composer__container__preview__container__options" id="options-c4d03092-a619-4ec1-8eed-5710d4b87f6d_reason">\
																			<textarea class="form-control textbox avm_accessible_txt_box_helper" aria-label="Can you briefly provide a reason for this rating?" placeholder="" value="" id="feedback_contents"></textarea>\
																		</div>\
																	</div>\
																</div>\
																<button type="button" class="btn btn-outline-primary default_card_submit w-100" role="button" aria-label="Submit" id="submit_btn">Submit</button>\
															</div>\
														</div>\
													</div>\
												</div>\
...
function sendFeedback() {
  ...
  fetch(url + "/api/gauge", {
    ...
    body: JSON.stringify({
      userid: userid,
      feedback: parseInt(feedback_type),
      note: feedback_content,
    }),
  })
    .then((response) => response.json())
    .then((response) => {
